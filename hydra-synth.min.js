(function(f){if(typeof exports==="object"&&typeof module!=="undefined"){module.exports=f()}else if(typeof define==="function"&&define.amd){define([],f)}else{var g;if(typeof window!=="undefined"){g=window}else if(typeof global!=="undefined"){g=global}else if(typeof self!=="undefined"){g=self}else{g=this}g.Hydra=f()}})(function(){var define,module,exports;return function(){function r(e,n,t){function o(i,f){if(!n[i]){if(!e[i]){var c="function"==typeof require&&require;if(!f&&c)return c(i,!0);if(u)return u(i,!0);var a=new Error("Cannot find module '"+i+"'");throw a.code="MODULE_NOT_FOUND",a}var p=n[i]={exports:{}};e[i][0].call(p.exports,function(r){var n=e[i][1][r];return o(n||r)},p,p.exports,r,e,n,t)}return n[i].exports}for(var u="function"==typeof require&&require,i=0;i<t.length;i++)o(t[i]);return o}return r}()({1:[function(require,module,exports){const Output=require("./src/output.js");const loop=require("raf-loop");const Source=require("./src/hydra-source.js");const Mouse=require("mouse-change")(document.getElementById("hydra-canvas"),function(buttons,x,y){});const Audio=require("./src/lib/audio.js");const VidRecorder=require("./src/lib/video-recorder.js");const ArrayUtils=require("./src/lib/array-utils.js");const Sandbox=require("./src/eval-sandbox.js");const Generator=require("./src/generator-factory.js");class HydraRenderer{constructor({pb=null,width=1280,height=720,numSources=4,numOutputs=4,makeGlobal=true,autoLoop=true,detectAudio=false,enableStreamCapture=false,canvas,precision="mediump",extendTransforms={}}={}){ArrayUtils.init();this.pb=pb;this.width=width;this.height=height;this.renderAll=false;this._initCanvas(canvas);this.synth={time:0,bpm:30,width:this.width,height:this.height,fps:undefined,stats:{fps:0},speed:1,mouse:Mouse,render:this._render.bind(this),setResolution:this.setResolution.bind(this),update:dt=>{},hush:this.hush.bind(this)};this.timeSinceLastUpdate=0;this._time=0;let precisionOptions=["lowp","mediump","highp"];let precisionValid=precisionOptions.includes(precision.toLowerCase());this.precision=precisionValid?precision.toLowerCase():"mediump";if(!precisionValid){console.warn('[hydra-synth warning]\nConstructor was provided an invalid floating point precision value of "'+precision+'". Using default value of "mediump" instead.')}this.extendTransforms=extendTransforms;this.saveFrame=false;this.captureStream=null;this.generator=undefined;this._initRegl();this._initOutputs(numOutputs);this._initSources(numSources);this._generateGlslTransforms();this.synth.screencap=()=>{this.saveFrame=true};if(enableStreamCapture){this.captureStream=this.canvas.captureStream(25);this.synth.vidRecorder=new VidRecorder(this.captureStream)}if(detectAudio)this._initAudio();if(autoLoop)loop(this.tick.bind(this)).start();this.sandbox=new Sandbox(this.synth,makeGlobal,["speed","update","bpm","fps"])}eval(code){this.sandbox.eval(code)}getScreenImage(callback){this.imageCallback=callback;this.saveFrame=true}hush(){this.s.forEach(source=>{source.clear()});this.o.forEach(output=>{this.synth.solid(1,1,1,0).out(output)})}setResolution(width,height){this.canvas.width=width;this.canvas.height=height;this.width=width;this.height=height;this.o.forEach(output=>{output.resize(width,height)});this.s.forEach(source=>{source.resize(width,height)});this.regl._refresh();console.log(this.canvas.width)}canvasToImage(callback){const a=document.createElement("a");a.style.display="none";let d=new Date;a.download=`hydra-${d.getFullYear()}-${d.getMonth()+1}-${d.getDate()}-${d.getHours()}.${d.getMinutes()}.${d.getSeconds()}.png`;document.body.appendChild(a);var self=this;this.canvas.toBlob(blob=>{if(self.imageCallback){self.imageCallback(blob);delete self.imageCallback}else{a.href=URL.createObjectURL(blob);console.log(a.href);a.click()}},"image/png");setTimeout(()=>{document.body.removeChild(a);window.URL.revokeObjectURL(a.href)},300)}_initAudio(){const that=this;this.synth.a=new Audio({numBins:4})}_initCanvas(canvas){if(canvas){this.canvas=canvas;this.width=canvas.width;this.height=canvas.height}else{this.canvas=document.createElement("canvas");this.canvas.width=this.width;this.canvas.height=this.height;this.canvas.style.width="100%";this.canvas.style.height="100%";this.canvas.style.imageRendering="pixelated";document.getElementById("hydra-canvas").appendChild(this.canvas)}}_initRegl(){this.regl=require("regl")({canvas:this.canvas,pixelRatio:1});this.regl.clear({color:[0,0,0,1]});this.renderAll=this.regl({frag:`
...
`;if(obj.type==="combine"||obj.type==="combineCoord")obj.inputs.unshift({name:"color",type:"vec4"});return Object.assign({},obj,{glsl:glslFunction})}else{console.warn(`type ${obj.type} not recognized`,obj)}}module.exports=GeneratorFactory},{"./glsl-source.js":13,"./glsl/glsl-functions.js":15}],13:[function(require,module,exports){const generateGlsl=require("./glsl-utils.js").generateGlsl;const formatArguments=require("./glsl-utils.js").formatArguments;const utilityGlsl=require("./glsl/utility-functions.js");var GlslSource=function(obj){this.transforms=[];this.transforms.push(obj);this.defaultOutput=obj.defaultOutput;this.synth=obj.synth;this.type="GlslSource";this.defaultUniforms=obj.defaultUniforms;return this};GlslSource.prototype.addTransform=function(obj){this.transforms.push(obj)};GlslSource.prototype.out=function(_output){var output=_output||this.defaultOutput;var glsl=this.glsl(output);this.synth.currentFunctions=[];if(output)try{output.render(glsl)}catch(error){console.log("shader could not compile",error)}};GlslSource.prototype.glsl=function(){var self=this;var passes=[];var transforms=[];this.transforms.forEach(transform=>{if(transform.transform.type==="renderpass"){console.warn("no support for renderpass")}else{transforms.push(transform)}});if(transforms.length>0)passes.push(this.compile(transforms));return passes};GlslSource.prototype.compile=function(transforms){var shaderInfo=generateGlsl(transforms);var uniforms={};shaderInfo.uniforms.forEach(uniform=>{uniforms[uniform.name]=uniform.value});var frag=`
  ...
  ${Object.values(utilityGlsl).map(transform=>{return`
            ${transform.glsl}
          `}).join("")}

  ${shaderInfo.glslFunctions.map(transform=>{return`
